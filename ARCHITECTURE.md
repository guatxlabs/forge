...
Forge est l'**antithèse offensive** du SOC blue-team **Plume**
([`guatxlabs/plume`](https://github.com/guatxlabs/plume)). Décision de design
...
La console consomme `core` via une **dép `git` publique épinglée** (`tag = "v0.2.2"`, récupérée au
...
**construit Forge d'abord** (Python, comme le prototype interne d'origine), puis on remontera les ~70 %
...
- `Scope` ← le scope-guard du prototype interne d'origine (appartenance fail-closed +
  exploit/destructif default-deny),
...
- `Finding` ← le schéma de finding du prototype interne d'origine, + `mitre`/`status`.
- `report.py` ← la section transparence du rapport du prototype interne d'origine.

## Les modules (P2) — outils autonomes orchestrés

Le contrat `Module` (`dry()`/`fire()`) enveloppe l'existant, qui **reste lançable seul** :
- une **collection de testeurs web** one-off (SSRF, IDOR, SQLi, XXE, JWT, GraphQL…) à CLI uniforme.
- le **prototype interne d'origine** — recon (nmap/httpx), `access_control` (IDOR 2-comptes),
  détection d'origine (CDN-bypass PoC-vérifié) ; + son planner coverage-safe (FLOOR sur les classes
  payantes) et son graph.
- un **service externe d'automatisation navigateur** — évasion : clic assisté par vision (Turnstile
  interactif), capture/replay (WAF qui bloque les XHR), intercept/modify (IDOR GraphQL
  persisted-query), drag OS.
- un **index vectoriel de findings** — mémoire : dedup, knowledge base, dispatch par technique.
...
La spécification de couverture MITRE ATT&CK / BAS de Plume est **déjà** posée côté bleu — Forge en
est la **moitié exécutante** :
...
- **P2 (fait)** — planner coverage-safe, runner, cerveau (`Brain`/`HeuristicBrain`, seam Claude),
  **graphe d'engagement** (`graph.py` : world-model hosts→services→findings), handlers réels
  ...
  store JSONL + dedup), run-records ATT&CK + `forge campaign`. Les briques les plus abouties du
  prototype interne d'origine (planner, runner, graph, access_control, origin) sont **portées
  proprement** dans le contrat Forge (pas d'import d'un projet voisin). Reste en durcissement : un
  backend à embeddings derrière l'interface `Memory`.
