...
| `FORGE_IMPORT_TIMEOUT` | Budget (s) du parse d'un **import de scan** (`POST /api/import`) — distinct de `FORGE_ENGINE_TIMEOUT` parce qu'un export de scanner (jusqu'à 64 Mio, le plafond d'entrée de la route) est légitimement plus long qu'un catalogue. Au dépassement : groupe tué, `504 import_timeout`, **aucun** finding partiel inséré. Relue à chaque appel. Valeur invalide ou `0` → défaut. | `600` | `120` |
