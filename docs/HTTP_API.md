...
| `GET /api/notify/sla` | **SLA de triage** : politique (budgets par sévérité) + `enabled` + `overdue_now` — un **aperçu en lecture seule** du nombre de findings actuellement en retard (il **ne notifie personne**). |
