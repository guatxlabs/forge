...
- **Rendre le binaire dispo — 3 voies** (détail `docs/TOOLS.md §5`) : (a) **déjà dans l'image `full`** (nmap/curl/dig/httpx/nuclei/subfinder) ; (b) **image custom mince** `FROM forge:0.0.1` + `apt install`/`COPY` (jeu figé, prod) ; (c) **monter SANS rebuild** — dossiers hôte `:ro` OPT-IN (commentés dans `docker-compose.yml`) : `./tools`→`/opt/tools` (binaires/scripts exécutables, **déjà sur le PATH**), `./plugins` (`FORGE_PLUGINS`, modules Python `@register` — code, haute confiance), `./toolspecs` (`FORGE_TOOLSPECS`, ToolSpecs déclaratifs gouvernés). Ex. : `cp myfuzzer forge/tools/ && chmod +x` puis décommenter `- ./tools:/opt/tools:ro`. ⚠️ `./tools`/`./plugins` = **opérateur-de-confiance** (arbitraire) ; la gouvernance ToolSpec borne l'invocation, pas l'interne du binaire.
