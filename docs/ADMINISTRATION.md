...
| **Transport `https://` vérifié** | seam TLS unique (`console/src/tls.rs`) — chaîne (racines Mozilla **+ ancres d'entreprise** `FORGE_EXTRA_CA_PEM`), nom d'hôte **et** validité vérifiés, handshake abouti **avant** le premier octet | Slack / Teams / PagerDuty sont joignables ; **aucune** option n'accepte un certificat non fiable |
| **Pas de credential en clair sur un réseau public** | règle **PARTAGÉE** (`net::reject_cleartext_secret`) avec le fetcher de source de détection : un jeton + une cible **publique** en `http://` = **refus**, qu'**aucune** variable n'ouvre ; en `https://` = autorisé (le jeton est protégé) ; en `http://` vers un collecteur **interne** autorisé = inchangé | passer en `https://` (une AC privée se fournit via `FORGE_EXTRA_CA_PEM`), viser un collecteur interne (`FORGE_ALLOW_INTERNAL_INTEGRATIONS=1`), ou retirer le jeton |
| **Jeton chiffré AU REPOS** | scellé dans `settings.notify_channel` par le chiffrement de champ (`FORGE_FIELD_KEY`, build par défaut) ; ouvert au **point d'usage unique** (`deliver_blocking`) | sans clé, l'écriture est refusée (**503** `field_key_missing`) et l'envoi **refuse de partir** plutôt que de se présenter **sans authentification** |
