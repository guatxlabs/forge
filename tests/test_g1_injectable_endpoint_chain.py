...
class TestBrainChainsInjectionOracles(unittest.TestCase):
    ...
    def test_fanout_bounded_one_param_per_endpoint(self):
        # BORNE : un SEUL paramètre (le 1er) par endpoint -> pas d'explosion sur une URL multi-params.
        g = EngagementGraph()
        g.add_host("app.test", kind="url")
        ep = "http://app.test/x?a=1&b=2&c=3"
        g.add_finding(_disc_endpoint(ep))
        sqli = [a for a in HeuristicBrain().propose(g) if a.kind == "sqli.probe" and a.target == ep]
        self.assertEqual(len(sqli), 1)
        self.assertEqual(sqli[0].params.get("param"), "a")     # 1er param seulement
