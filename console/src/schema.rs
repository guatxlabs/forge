...
CREATE TABLE IF NOT EXISTS module(
  ...
  -- SCHÉMA DE PARAMS (JSON) servi à l'UI (formulaire de lancement dynamique) et allowlist de drapeaux
  -- pour les extra_args : SONDÉS depuis le registre Python (`forge modules --json`), re-écrits au re-probe.
  params_schema TEXT DEFAULT '[]', flag_allowlist TEXT DEFAULT '[]',
  -- OUTIL AJOUTÉ PAR L'UTILISATEUR (POST /api/tools) : 1 = ToolSpec déclaratif déposé par un admin dans le
  -- dossier server-managed (jamais un module natif). Sert à LISTER (GET /api/tools) et à GARDER la
  -- suppression (DELETE /api/tools/:kind ne retire QUE les user_added=1 — jamais un built-in). Jamais
  -- réécrit par populate_modules (no-clobber comme enabled/web_allowed) ; re-marqué au boot depuis le dir.
...
-- NOTIFICATION (triage enrichi) : boîte de réception PERSONNELLE, in-app, de collaboration sur les
-- findings. Une ligne = « quelque chose qui te concerne » : `user_id` = RECIPIENT (users.id), `kind` ∈
-- {finding.assigned|finding.triage}, `engagement_id`+`finding_id` = la ressource liée (l'UI en dérive le
-- lien profond), `text` = résumé humain PLAIN (échappé À L'AFFICHAGE — jamais interprété comme HTML),
-- `read` (0/1) = lu/non-lu, `created` = horodatage. GRANT-SCOPED À L'ÉCRITURE (notifications.rs : un
-- destinataire ne reçoit une notif QUE s'il a un grant sur l'engagement — miroir de l'assignation) ;
-- LECTURE/MARQUAGE fail-closed au `user_id` de l'appelant (jamais les lignes d'un autre). Index sur
-- (user_id, read) : la vue « mes non-lues d'abord » + le compteur non-lu sont O(log n). ADDITIVE + INERTE
-- tant qu'aucune assignation/triage n'a lieu -> community/défaut byte-identique.
...
CREATE TABLE IF NOT EXISTS module(
  ...
  params_schema TEXT DEFAULT '[]', flag_allowlist TEXT DEFAULT '[]',
  -- user_added : miroir PG de la colonne SQLite (outil ajouté par l'UI, cf. SCHEMA). Inline (comme
  -- params_schema/flag_allowlist) : un cluster PG FRAIS l'obtient ; un cluster ancien via migrate-store.
  user_added BIGINT DEFAULT 0);
CREATE TABLE IF NOT EXISTS roe_decision(
...
CREATE TABLE IF NOT EXISTS engagement(
  ...
  -- POLITIQUE RÉSEAU (privé/LAN/loopback) — opt-in PAR ENGAGEMENT (miroir PG de la colonne SQLite,
  -- cf. migrate()). DEFAULT 0 = FAIL-CLOSED. Effectif = master global AND cette colonne (run_create).
  allow_private BIGINT NOT NULL DEFAULT 0);
CREATE TABLE IF NOT EXISTS finding_template(
...
CREATE TABLE IF NOT EXISTS presence(
  conn_id TEXT PRIMARY KEY, login TEXT NOT NULL, role TEXT NOT NULL,
  engagement_id BIGINT, instance_id TEXT, since BIGINT, last_seen BIGINT);
-- NOTIFICATION (triage enrichi) — MIROIR PG de la table SQLite. Boîte de réception personnelle in-app
-- (recipient=user_id, kind, engagement_id/finding_id liés, text plain échappé à l'affichage, read 0/1).
-- Grant-scopée à l'écriture, fail-closed au user_id de l'appelant en lecture/marquage. INERTE tant
-- qu'aucune assignation/triage n'émet -> byte-identique par défaut.
CREATE TABLE IF NOT EXISTS notification(
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id BIGINT NOT NULL, kind TEXT NOT NULL,
...
pub(crate) fn migrate(db: &Connection) {
    let alters = [
        ...
        // SCHÉMA DE PARAMS + allowlist de drapeaux (JSON) : servis à l'UI (formulaire dynamique) et
        // à la validation server-side des extra_args. Additifs (DEFAULT '[]') ; SONDÉS par populate_modules.
        "ALTER TABLE module ADD COLUMN params_schema TEXT DEFAULT '[]'",
        "ALTER TABLE module ADD COLUMN flag_allowlist TEXT DEFAULT '[]'",
        // user_added (POST /api/tools) : intention serveur, jamais réécrite par le re-probe (no-clobber),
        // re-marquée au boot depuis le dossier server-managed. Additive (DEFAULT 0 = built-in).
        "ALTER TABLE module ADD COLUMN user_added INTEGER DEFAULT 0",
        ...
        // POLITIQUE RÉSEAU (privé/LAN/loopback) — opt-in PAR ENGAGEMENT. DEFAULT 0 (FAIL-CLOSED) => une base
        // ANTÉRIEURE voit TOUS ses engagements existants refuser le scan de cibles privées tant que
        // l'opérateur ne l'active pas explicitement (+ le master global). Effectif = master global AND cette
        // colonne (calculé server-side dans run_create). NOT NULL autorisé (DEFAULT constant). Additif/error-ignored.
        "ALTER TABLE engagement ADD COLUMN allow_private INTEGER NOT NULL DEFAULT 0",
    ];
    for a in alters {
    ...
    // NOTIFICATION (triage enrichi) : re-créée ici (idempotent, CREATE IF NOT EXISTS) en plus du SCHEMA,
    // pour qu'une base ANTÉRIEURE à son introduction l'obtienne au 1er boot suivant la mise à jour (même
    // discipline que presence/saved_view). Table NEUVE (pas un ALTER de colonne) : le migrateur Stage-3
    // (énumération de sqlite_master) la couvre automatiquement. ADDITIVE + INERTE tant qu'aucune
    // assignation/triage n'émet -> byte-identique. error-ignored.
    let _ = db.execute(
        "CREATE TABLE IF NOT EXISTS notification(
           id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, kind TEXT NOT NULL,
           engagement_id INTEGER, finding_id INTEGER, text TEXT NOT NULL DEFAULT '',
    ...
    let _ = db.execute("CREATE INDEX IF NOT EXISTS idx_notification_user_read ON notification(user_id, read)", []);
...
pub(crate) fn populate_modules(store: &crate::store::Store) {
    ...
    for m in &mods {
        ...
        // SCHÉMA DE PARAMS + allowlist de drapeaux (JSON, sérialisés compacts). Absents => '[]' (défaut sûr).
        let params_schema = m.get("params_schema").map(|v| v.to_string()).unwrap_or_else(|| "[]".to_string());
        let flag_allowlist = m.get("flag_allowlist").map(|v| v.to_string()).unwrap_or_else(|| "[]".to_string());
...
pub(crate) fn upsert_probed_module(store: &crate::store::Store, kind: &str, exploit: bool, destructive: bool,
                        available: bool, mitre: &str, descr: &str, params_schema: &str, flag_allowlist: &str) {
    let web_allowed = module_web_allowed(kind, exploit, destructive);
    // params_schema/flag_allowlist SONT sondés (comme exploit/mitre/descr) : ré-écrits au re-probe pour
    // suivre l'évolution du registre Python. L'intention opérateur (enabled/available_override/web_allowed)
    // reste ABSENTE du SET (no-clobber) — cf. régression connecteur.
    let _ = store.execute(
        "INSERT INTO module(kind,exploit,destructive,available,mitre,descr,web_allowed,params_schema,flag_allowlist)
         VALUES(?,?,?,?,?,?,?,?,?)
         ON CONFLICT(kind) DO UPDATE SET exploit=excluded.exploit, destructive=excluded.destructive,
           ...
           params_schema=excluded.params_schema, flag_allowlist=excluded.flag_allowlist",
        &crate::sql_params![kind, exploit as i64, destructive as i64, available as i64, mitre, descr,
                            web_allowed as i64, params_schema, flag_allowlist],
    );
}
