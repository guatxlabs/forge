...
class IdorDifferential(Module):
    ...
    @classmethod
    def _fetch(cls, url, headers, timeout=15, method="GET", body=None):
        """Retourne (status, body, content_type). content_type cadre la comparaison : deux corps
        de types différents (html vs json) ne sont jamais « le même objet ». body peut être None."""
        data = body.encode("utf-8") if isinstance(body, str) else body
        req = urllib.request.Request(url, headers=headers or {}, method=method, data=data)
        try:
            with urllib.request.urlopen(req, timeout=timeout) as r:
                ct = (r.headers.get("Content-Type") or "").split(";")[0].strip().lower()
                return r.status, r.read(200000).decode("utf-8", "replace"), ct
    ...
    def _fire_read(self, action, A, B, urls):
        ...
        for url in urls:
            ...
            findings.append(self.finding(
                ...
                category="CWE-639", cwe="CWE-639", mitre="T1190",
                fix=("Contrôle d'ownership côté serveur : vérifier que l'utilisateur authentifié possède "
                ...
                status=("vulnerable" if vuln else "tested"),
                tool="forge/modules/web.py:access_control.idor",
    ...
    def _fire_write(self, action, A, B, urls, method):
        ...
        for url in urls:
            ...
            findings.append(self.finding(
                ...
                category="CWE-639", cwe="CWE-639", mitre="T1190",
                fix=("Contrôle d'ownership côté serveur sur les écritures : vérifier que l'utilisateur "
                ...
                evidence=(f"{method} B={wb[0]} accepté={b_accepted} ; A avant={before[0]}/"
                          ...
                          f"muté={mutated}"),
                poc=self._curl(url, B.get("headers", {}), method=method, body=body)))
        return findings

    @staticmethod
    def _curl(url, headers, method="GET", body=None):
