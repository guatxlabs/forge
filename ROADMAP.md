...
- **Schema-driven custom tool arguments in the UI** (`0d19512`, `5c15dfd`) — every tool-driving module (nmap fully-custom + nuclei + the 20 ToolSpec catalog + ffuf/sqlmap/httpx/subfinder natives = 26 kinds) carries a typed `params_schema` + a conservative `flag_allowlist`; the Launch view renders each tool's params form dynamically, incl. a safe allowlisted `extra_args` (no shell, `--`-guard, output/config/proxy/exfil flags rejected fail-closed).
- **Rate-limit control** (`3034616`) — per-run `rate` + `scope.rate` → an engine throttle (token-bucket in `Oracle._http`, all active-HTTP kinds) + per-tool rate flags (nmap `--max-rate`, nuclei/httpx `-rl`, naabu/masscan `--rate` [replaced hard-coded 1000], feroxbuster `--rate-limit`, sqlmap/… `--delay`) + **429/`Retry-After`/WAF back-off** (bounded exponential) surfaced as a "rate-limited/blocked" run signal. Rate unset = byte-identical default.
- **Add custom tools from the UI** (`b6b8ee8`) — `POST /api/tools` (admin-gated, ledgered) takes a declarative governed **ToolSpec** (binary/docker, no-shell token argv, `params_schema`, `flag_allowlist`) → validated fail-closed (`custom.*` namespace, no built-in override, traversal-safe, interpreter/exfil-flag rejected, status clamped) → persisted to a managed dir + hot-reloaded → appears in Launch **configurable via its schema**, scope-guarded. UI form (`admin/addtool.js`) + `docs/TOOLS.md`. Arbitrary-code `.py` upload deliberately OFF (high-trust `FORGE_PLUGINS` only).
