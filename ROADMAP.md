...
- **G1 (`7b60fdd`+`8d91826`) — Exploitation autonome** : les oracles rce/sqli/idor/xss sortent « config manquante » faute de surface injectable. Chaîne **crawl (katana/gospider) → param-discovery (host:port/path?param=val nœuds chaînables) → oracles d'injection** qui testent le param découvert. + hook **cerveau LLM Ollama** optionnel (priorise params/payloads), fail-open/mockable/off par défaut. Cœur déterministe d'abord. *(gros levier)*
