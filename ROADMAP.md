...
## ✅ Publication open-source (org `guatxlabs`) — LIVRÉ (publié + durci, 2026-07-19)

**✅ ÉTAT : forge est PUBLIÉ sur `github.com/guatxlabs/forge`, public, durci et gouverné.**
FAIT : publication (`main` + tag `v0.0.1` signé ; historique 100 % `GuatX <noreply@guatx.com>` signé — **tags/notes/branches inclus**, ancien tag legacy purgé local+vps+gc) · standalone-buildable (git-dep `guatxlabs/core@v0.1.0`, `Cargo.lock` en source git, la CI résout la VRAIE dép = ce qu'un clone fait) · licence **AGPL-3.0-or-later** (alignée core LGPL-or-later + ocular) · **ruleset actif** (PR + signatures + linéaire + squash-only + CI verte + CodeQL High/Critical, bypass-list vide = admins inclus) · **CodeQL** vert · **secret scanning + push protection** (+ `.github/secret_scanning.yml` sur les fixtures) · **Dependabot** (9 PR traitées) · **CI** = tests + clippy + cargo-audit + **gitleaks binaire épinglé** (l'action retirée : exigeait GITHUB_TOKEN sur les PR) + job agrégat `CI` à nom stable.
RÉGLÉ EN ROUTE (imprévus attrapés) : advisory **jsonwebtoken → 10.4.0 + backend `rust_crypto`** (openssl-free ; rsa Marvin RUSTSEC-2023-0071 ignoré justifié = vérif clé PUBLIQUE only) · fix API **getrandom→fill** (0.4) · main cassé/réparé une fois (merge d'une PR au CI rouge sans ruleset — d'où le ruleset).
⚠️ **WORKFLOW MAINTENANT : PR obligatoire sur `main` public** (push direct bloqué par le ruleset) ; `vps` reste le mirror privé (push direct OK).
**RESTE UNIQUEMENT des enhancements produit OPTIONNELS** (cf. « 🔜 RESTE À FAIRE ») : cerveau LLM Ollama, re-tir lab.guatx.com (oracles derrière CF via `evasion.discover`), mode origine/tunnel SSH→Traefik. **Le splash Reddit/X est réservé au moment purple-team Forge+Plume.**

*(Ci-dessous : le plan & les décisions d'origine, conservés comme trace — désormais tous exécutés.)*
...
- *✅ Fait côté GitHub (domaine user, settings)* : **ruleset** sur `main` (deletion + non-fast-forward + required_signatures + linéaire + squash-only + required_status_checks `CI` + **CodeQL High/Critical** ; bypass-list vide = admins inclus — sur un *ruleset* c'est le défaut, ≠ l'ancienne case « Include administrators » des branch-protections) ; **secret scanning + push protection** (+ `.github/secret_scanning.yml` pour les fixtures ; le hook VPS reste pour les remotes privés) ; **CodeQL** default setup (python+javascript ; Rust couvert par clippy+cargo-audit CI) ; **Dependabot** alertes+MàJ ; **désactiver wiki/projects** si inutilisés. *Restent au choix de l'user (badge « Verified »)* : enregistrer la clé ed25519 comme _Signing Key_ + vérifier `noreply@guatx.com` (sinon commits signés = « Unverified ») ; Private Vulnerability Reporting (canal SECURITY.md).
