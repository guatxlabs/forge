...
## 🔭 Chantiers restants (issus des tirs comparatifs A→E4 ; consignés avant exécution)
...
- **G1 — Exploitation autonome** : les oracles rce/sqli/idor/xss sortent « config manquante » faute de surface injectable. Chaîne **crawl (katana/gospider) → param-discovery (host:port/path?param=val nœuds chaînables) → oracles d'injection** qui testent le param découvert. + hook **cerveau LLM Ollama** optionnel (priorise params/payloads), fail-open/mockable/off par défaut. Cœur déterministe d'abord. *(gros levier)*
- **G2 — Plancher nuclei** : `-severity medium+` masque les expos INFO/LOW (Swagger/openapi, LLM panels, exposed-files). Élargir/rendre configurable, sévérité réelle du finding préservée (pas d'inflation).
- **G3 — Parallélisme intra-vague** : executor série → gros run >25 min. Paralléliser l'exécution (borné) en **sérialisant les écritures ledger/ingest** (ordre déterministe — raison du report par E3). Gouvernance intacte, composé D1/E2/E3/E4. *(le plus risqué)*
- **G4 — Flake pytest** : race timing ROE-arming/durability (`test_roe`/`test_connector_governance`/`test_engine_durability`, ~1/5-8, corrélé charge — hostnames `.test` qui VETO par intermittence en résolution DNS). Rendre déterministe (mock résolution / éliminer dépendance temps).

Orchestration : worktrees isolés (anti-course) ; G3 après G1 (même executor).
